...
# 评测镜像（builtin / DeepEval）默认不随服务启动，且依赖较重，
...
The add-on package contains the two evaluation images in one deduplicated
archive, plus tooling and a loader script.
EOF
}
...
加载后，按主仓库文档运行 builtin / DeepEval 评测。
