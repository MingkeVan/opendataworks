...
def run_case(base_url: str, case: dict[str, Any], args: argparse.Namespace, judge_config: JudgeConfig) -> dict[str, Any]:
    ...
    try:
        ...
        # Reference SQL is evaluation evidence, not a prerequisite for running
        # the Agent. Execute it only after the task and persisted messages have
        # been collected so a broken reference-query channel cannot leave an
        # empty topic with no submitted user message.
