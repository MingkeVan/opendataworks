<template>
  <div class="v2-workbench" :class="{ 'artifacts-open': artifactsPanelOpen && !isWidgetMode }">
    ...
    <aside v-if="artifactsPanelOpen && !isWidgetMode" class="v2-artifacts-panel">
      ...
      <el-scrollbar class="v2-artifacts-scroll">
        ...
        <div v-else class="v2-artifact-tree">
          <div
            v-for="row in artifactRows"
            :key="row.type === 'dir' ? 'd:' + row.path : 'f:' + row.file.rel_path"
            class="v2-artifact-row"
            :class="row.type"
          ...
          >
            <button
              v-if="row.type === 'dir'"
              type="button"
              class="v2-artifact-dir"
              @click="toggleArtifactDir(row.path)"
            >
              <svg class="v2-artifact-chevron" :class="{ open: !collapsedArtifactDirs.has(row.path) }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="10" height="10"><polyline points="9 18 15 12 9 6" /></svg>
              ...
              <span class="v2-artifact-dir-name">{{ row.name }}</span>
            </button>
            <div v-else class="v2-artifact-file" @click="openArtifact(row.file)">
              <svg class="v2-artifact-file-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6" /></svg>
              <span class="v2-artifact-name" :title="row.name">{{ row.name }}</span>
              <span class="v2-artifact-meta">
                <span class="v2-artifact-tag" :class="row.file.kind">{{ row.file.kind === 'input' ? '上传' : '生成' }}</span>
                <span class="v2-artifact-size">{{ formatBytes(row.file.size) }}</span>
              </span>
              <span class="v2-artifact-row-actions">
                <button
                  type="button"
                  class="v2-artifact-row-btn"
                  title="预览"
                  aria-label="预览"
                  @click.stop="openArtifact(row.file)"
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7Z" /><circle cx="12" cy="12" r="3" /></svg>
                </button>
                <a
    ...
    <!-- Preview modal: the sole preview surface, opened directly from either
         the chat message attachment cards or the artifact tree panel. -->
    <Teleport to="body">
      <div
...
const artifacts = ref([])
const collapsedArtifactDirs = ref(new Set())
// Nested tree built from each file's rel_path segments; root-level files
// (no '/') stay as depth-0 leaves rather than a synthetic '/' folder.
const artifactTree = computed(() => {
  const root = { children: new Map() }
  for (const file of artifacts.value) {
    const parts = String(file.rel_path || '').split('/')
    let node = root
    let path = ''
    for (let i = 0; i < parts.length - 1; i++) {
      path = path ? `${path}/${parts[i]}` : parts[i]
      if (!node.children.has(parts[i])) {
        node.children.set(parts[i], { type: 'dir', name: parts[i], path, children: new Map() })
      }
      node = node.children.get(parts[i])
    }
    const fileName = parts[parts.length - 1]
    node.children.set(`file:${fileName}`, { type: 'file', name: fileName, file })
  }
  return root
})
// Flattens the tree into rows with a depth, skipping children of collapsed dirs.
const artifactRows = computed(() => {
  ...
  function walk(node, depth) {
    ...
    for (const dir of dirs) {
      rows.push({ type: 'dir', name: dir.name, path: dir.path, depth })
      if (!collapsedArtifactDirs.value.has(dir.path)) walk(dir, depth + 1)
    }
    for (const file of files) {
      rows.push({ type: 'file', name: file.name, file: file.file, depth })
    }
  }
  walk(artifactTree.value, 0)
  return rows
})
function toggleArtifactDir(path) {
  const next = new Set(collapsedArtifactDirs.value)
  if (next.has(path)) next.delete(path)
  else next.add(path)
  collapsedArtifactDirs.value = next
}
...
/* ── Artifact tree (right panel file browser) ────────────────────────────── */
.v2-artifact-tree { display: flex; flex-direction: column; }
.v2-artifact-row { border-bottom: 1px solid #F1F3F6; }
.v2-artifact-dir {
  display: flex; align-items: center; gap: 6px; width: 100%;
  padding: 8px 14px 8px 0; border: none; background: transparent; cursor: pointer;
  text-align: left; font-size: 11px; font-weight: 600; color: #6B7280;
  text-transform: uppercase;
}
.v2-artifact-dir:hover { color: #4338CA; }
.v2-artifact-chevron { flex: none; color: #9AA4B2; transition: transform 0.15s ease; }
.v2-artifact-chevron.open { transform: rotate(90deg); }
.v2-folder-icon { color: #9AA4B2; flex: none; }
.v2-artifact-dir-name { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.v2-artifact-file {
  display: flex; align-items: center; gap: 8px; width: 100%;
  padding: 10px 14px 10px 0; cursor: pointer;
}
.v2-artifact-file:hover { background: #F2F5F9; }
.v2-artifact-file-icon { flex: none; color: #4F46E5; }
.v2-artifact-name {
...
.v2-artifact-row-actions { flex: none; display: flex; align-items: center; gap: 2px; }
.v2-artifact-row-btn {
  display: inline-flex; align-items: center; justify-content: center;
  width: 24px; height: 24px; border: none; border-radius: 6px;
  background: transparent; color: #6B7280; cursor: pointer;
}
.v2-artifact-row-btn:hover { background: #E6EAF0; color: #111827; }
...
/* ── Message attachments (generated files: preview + download) ───────────── */
.v2-msg-attachments { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
.v2-msg-attachment {
...
.v2-msg-attachment.is-clickable { cursor: pointer; }
.v2-msg-attachment:hover { background: #EEF2F7; border-color: #D4DCE6; }
.v2-msg-attachment-icon { flex: none; color: #4F46E5; }
.v2-msg-attachment-name {
  min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.v2-msg-attachment-size { flex: none; font-size: 11px; color: #9AA4B2; }
.v2-msg-attachment-actions { flex: none; display: flex; align-items: center; gap: 2px; }
.v2-msg-attachment-btn {
  display: inline-flex; align-items: center; justify-content: center;
  width: 22px; height: 22px; border: none; border-radius: 6px;
  background: transparent; color: #6B7280; cursor: pointer;
}
.v2-msg-attachment-btn:hover { background: #E6EAF0; color: #111827; }
