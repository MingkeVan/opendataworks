          ...
          <template v-for="msg in activeMessages" :key="msg.id">
            ...
            <div v-else class="v2-msg-row v2-msg-assistant">
              <div class="v2-assistant-body">
                ...
                <div v-if="msg.status === 'streaming' && !msg.thinkingText && !cleanMainText(msg) && !msg.tools.length" class="v2-loading">
                ...
                <div v-if="cleanMainText(msg)" class="v2-main-text">
                  {{ cleanMainText(msg) }}<span v-if="msg.status === 'streaming'" class="v2-cursor">▋</span>
                </div>
                ...
                <!-- Done marker -->
                <div v-if="msg.status === 'success' && msg.mainText" class="v2-done-mark">
                  <span class="v2-done-icon">✅</span>
                  <span>Done</span>
                </div>
              </div>
            </div>
...
const formatToolData = (v) => {
  if (v === null || v === undefined) return ''
  if (typeof v === 'string') return v.length > 2000 ? v.slice(0, 2000) + '...' : v
  try { const s = JSON.stringify(v, null, 2); return s.length > 2000 ? s.slice(0, 2000) + '...' : s } catch (_) { return String(v) }
}
...
const extractSkillLabel = (tool) => {
  ...
  if (skill) return `Read ${skill.replace(/^.*\//, '')} skill`
  return String(tool.name || 'Skill')
}
...
.v2-messages-inner { max-width: 780px; margin: 0 auto; padding: 28px 24px 36px; }
...
/* ═══ Main text ═══ */
.v2-main-text { white-space: pre-wrap; line-height: 1.75; color: var(--text); font-size: 14.5px; }
...
/* ═══ Done marker ═══ */
.v2-done-mark { margin-top: 10px; display: inline-flex; align-items: center; gap: 6px; font-size: 13px; color: var(--text-muted); }
.v2-done-icon { font-size: 14px; }

/* ═══ Composer ═══ */
.v2-composer-wrap { padding: 12px 24px 18px; flex-shrink: 0; background: var(--bg); }
.v2-composer { max-width: 780px; margin: 0 auto; border: 1px solid var(--border); border-radius: 16px; background: var(--bg-card); box-shadow: 0 1px 6px rgba(0,0,0,0.05); overflow: hidden; }
