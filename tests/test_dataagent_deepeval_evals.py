    ...
    blocks = [
        {
            "type": "tool_use",
            "tool_name": "Read",
            "output": "参考文档：TODO {target_date} 未找到匹配对象。不要使用 OpenDataWorks 平台元数据或 SELECT *。",
        }
    ]
...
def test_deepeval_run_case_submits_turns_in_order(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: execution_order.append("reference") or {"applicable": False, "passed": None},
    )
...
def test_deepeval_reference_query_transport_failure_has_specific_reason(monkeypatch):
    _install_fake_deepeval(monkeypatch)
    runner = _load_runner()
    case = _sample_case()
