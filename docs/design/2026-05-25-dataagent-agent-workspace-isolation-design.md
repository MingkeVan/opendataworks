...
The hook also allows the current run's per-project Claude data dir, `<config_dir>/projects/<encoded_cwd>` (where `config_dir` is `$CLAUDE_CONFIG_DIR` or `$HOME/.claude`, and `encoded_cwd` is `realpath(cwd)` with every non-alphanumeric char replaced by `-`). When a tool result exceeds the inline size limit, Claude Code offloads it to `<config_dir>/projects/<encoded_cwd>/<session_id>/tool-results/<tool_use_id>.{txt,json}` and instructs the agent to `Read` that path; without this allowance the follow-up read is denied as "outside workspace". The allowance is scoped to the current workspace's encoded cwd, so it covers only this run's own transcripts and offloaded results and never another topic's data or shared credentials, even when `HOME` is shared across topics in the default in-process mode.
