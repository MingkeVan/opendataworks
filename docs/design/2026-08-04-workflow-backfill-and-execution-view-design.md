...
| 列 | 全部列 | **只省掉「工作流」**——整屏都是同一个工作流，这一列是噪音；其余列（含来源、错误信息）保持一致 |
| 筛选栏 | 工作流下拉 + 快捷状态 | 无 |
| 统计卡 | 有 | 无 |
| 空状态文案 | 「当前筛选条件下暂无工作流执行记录」 | 「暂无执行记录」 |
| 取数方式 | 项目级一次查询，取最近 50 条 | 按 `processDefinitionCode` 服务端过滤 |

列宽约定：窄的离散列（实例ID、状态、触发方式、来源、时长）固定宽度；「调度日期 / 开始时间 / 结束时间 / 错误信息 / 工作流」用 `min-width` 弹性吸收剩余宽度。**两处至少要各留有若干弹性列**，否则关掉弹性列的那一侧会因为全是固定宽度而撑不满容器、右侧留白。这正是「执行历史宽度没拉开」的成因。
