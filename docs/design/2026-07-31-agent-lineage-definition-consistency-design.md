...
**豁免仅限"本工作流内只有本任务写这张表"。** 若还有别的任务也写它，
那条读关系承载着"对方 → 本任务"的真实依赖边，缺了会让该边彻底消失
（`inferTaskEdges` 对没有任何读关系的任务直接 `continue`，它永远不会成为任何边的下游），
调度顺序可能出错。这种情况照常报告 `LINEAGE_SQL_RELATION_MISSING`，
消息单独说明原因。判定所需的 `writesByTask` 在工作流级检查里本就全量在手，无额外查询。

两层行为有意不同：

| 路径 | 行为 |
| --- | --- |
| 任务保存（`findHighConfidenceGap`） | 一律豁免 |
| 发布预检 / 导出（`collectSqlIssues`） | 仅当无其他任务写该表时豁免 |

保存路径拿不到工作流上下文，且被依赖的任务可能尚未创建（Agent 完全可能先建下游再建上游），
在那里阻断会变成顺序敏感的假失败。因此保存放行，由发布/导出兜底。
