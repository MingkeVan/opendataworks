...
## 5. 2026-07-20 助手欢迎页与路由上下文补充设计

本节替代 2.1 中“在对话顶栏直接放置普通智能体下拉框”的旧设计，适用于当前
`dataagent/dataagent-frontend/src/views/intelligence/NL2SqlChatV2.vue`。

### 5.1 进入条件与欢迎页

- `/chat` 没有 `agent_id` 时只展示助手欢迎页，不渲染会话侧栏、输入框、模型选择、历史消息，也不请求模型或话题接口。
- 欢迎页采用居中品牌区、欢迎文案、横向可换行的助手胶囊按钮和“选择上方助手开始对话”提示。
- 颜色沿用 OpenDataWorks 现有蓝色变量；默认助手只排序靠前并显示标记，不自动进入。
- 助手目录加载失败时保留 URL 并提供重试；目录为空时展示联系管理员的空状态。
- 用户选择助手后，以 `router.replace` 写入 `agent_id`，清除遗留 `topic_id`、`message_id`，随后才初始化会话工作台。

### 5.2 对话内助手身份与切换

- 有效 `agent_id` 是渲染会话工作台的前提；左侧保留助手下拉框，并作为工作台内唯一的助手切换入口。
- 主区原会话标题顶栏改为静态展示当前助手名称；不展示助手描述、图标、切换箭头，也不响应点击。会话标题仅保留在左侧历史列表。
- 从左侧助手下拉框切换时，若已有消息、草稿、附件或运行任务，切换前确认；文字草稿保留，已上传附件仍归原话题并从新会话清除，运行任务只 detach、不取消。
- URL 中的助手无效时清除 `agent_id`、`topic_id`、`message_id` 并回欢迎页，不自动回退默认助手。
- `agent_id` 与 `topic_id` 归属冲突时保留助手，清除话题定位，避免把一个助手的问题发送到另一个助手的话题。

### 5.3 路由与认证

- URL 是当前助手的唯一事实来源，不使用 localStorage。
- DataAgent 主菜单、Skill/智能体详情和返回操作统一只继承 `agent_id`；`topic_id`、`message_id` 不跨页面传播。
- 登录拦截、本地登录、OAuth、401 重新登录和主动退出均使用完整 `fullPath` 回跳；有 `agent_id` 恢复对话，无 `agent_id` 恢复欢迎页。
- 公开助手目录现有的名称、描述、默认标记和预置问题已经满足 UI 展示，不新增后端接口或数据字段。
