...
- 命令来源（单一主路径，两端一致）：
  - **技能命令**：来自当前 Agent 的 `skill_folders`，菜单直接展示技能名称。选中后把技能名补全为命令 token（`/<skill> `）保留在输入框，名称可见，用户补全问题后正常发送。
...
- `type==='skill'`：选中时把 `inputText` 设为 `insertText`（技能名命令 token `/<folder> `）并聚焦输入框，技能名保留可见，等待用户补全。
...
- 技能无原生命令协议，选择“补全技能名命令 token”而非新增后端调用契约，符合常见斜杠命令习惯（展示并保留技能名），保持零后端改动、低风险、两端一致。
- 技能名直接用技能目录名（Agent 与运行时的规范标识），避免依赖 admin 描述接口，嵌入式同样可用。
