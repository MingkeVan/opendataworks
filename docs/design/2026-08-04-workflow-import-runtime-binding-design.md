...
- 发布**开始前**就定下本次使用的环境（已绑定用绑定值，未绑定解析当前默认），整次部署只走这个显式 id，结束时把它固化回工作流。此前每处调用各自解析一次默认配置，管理员在部署中途切换会把同一次部署的 Dolphin 调用分散到两个环境，收尾时固化的也是切换后的环境而非实际部署的那个。未绑定的首次发布期间持全局锁，与配置变更互斥直到归属落库
