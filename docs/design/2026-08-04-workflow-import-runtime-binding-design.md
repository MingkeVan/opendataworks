...
配置对象本身不做进程内缓存。此前 `DolphinConfigService` 用一个 volatile 字段缓存，失效发生在事务内部：清空之后、事务提交之前，其他线程会从库里读到旧数据把缓存重新填上，提交后又没有第二次失效，缓存可能长期停留在旧 URL / 旧默认环境上。配置查询很轻，而它每个调用点后面紧跟着一次 Dolphin HTTP 调用，省这次 SELECT 不划算。
...
- `V51` 把已绑定运行态的空配置行回填到**有效默认环境**
- 发布编排在 `publish(deploy)` 的事务第一次读库前取得全局锁，审批通过也在读取发布记录前取锁；随后定下本次使用的环境（已绑定用绑定值，未绑定解析当前默认），整次部署只走这个显式 id，结束时把它固化回工作流。锁不能放在 `WorkflowDeployService.deploy()`：它加入 `publish()` 的外层事务时，自动保存已经读取并写入 `data_workflow`，RR 快照与配置归属都已确定，局部锁来不及保护前半段。是否首次部署也必须读工作流后才能判断，因此 `publish(deploy)` 入口统一串行；这是换取完整事务级正确性的有意取舍
- 任何配置写入路径（新建 / 修改 / 删除 / 设默认）在锁内动手之前，先把仍为空的运行态绑定固定到当下的有效默认环境。互斥只能挡住并发，挡不住"先切默认、之后的发布落到新环境"这种顺序漂移
- 绑定数量统计在目标是当前默认环境时额外计入空配置行，作为兜底
- 删除平台工作流时，存在性检查、调度下线、定义下线和定义删除全部显式传入该工作流的 `dolphin_config_id`；否则默认环境恰好存在同编码定义时会删错目标，并把真正绑定的定义留在原环境
- `switchSchedulerEngine` 在读取工作流前取得同一把锁，再把 `dolphin_config_id` 改为目标环境并清空旧运行态字段；否则配置删除可在切换事务落库前数到零并删除目标环境，留下悬空绑定

"有效默认环境"必须与运行时的 `DolphinConfigService.getDefaultConfig` 用同一套判定：优先显式 `is_default = 1`，没有显式默认时回落到最新的启用配置。迁移与切换逻辑都按这个定义，只认显式标记会漏掉"未设默认但工作流已经跟着某个配置跑"的状态。
...
上述 RESET 例外只针对导入提交；工作流之后真正执行 `publish(deploy)` 时仍会取得全局锁，因为发布可能创建新的 Dolphin 运行态绑定。
...
- 发布单测分别钉住 `publish(deploy)` 在工作流首次读取前取锁、审批通过在发布记录首次读取前取锁；删除回归用例钉住整条 Dolphin 清理链都使用工作流绑定的配置 id
- 调度环境切换单测钉住全局锁早于 `data_workflow` 的首次读取，保证与配置删除遵守同一锁顺序
- 本地 Spring Boot 测试已连接 MySQL 8 并由 Flyway 从 V48 实际迁移到 V51，三条迁移成功应用；这覆盖迁移启动验证，不替代上一条真实双事务并发测试
