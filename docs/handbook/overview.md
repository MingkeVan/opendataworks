...
| **OpenDataWorks DB** | 统一数据库 `opendataworks`，默认账号 `opendataworks/opendataworks123` | Flyway 首次启动自动建表，账号配置见 `deploy/.env.example` |
...
2. **初始化数据库**：在 MySQL 中创建 `opendataworks` 库和账号（参考 quick-start），后端启动时由 Flyway 自动建表。
